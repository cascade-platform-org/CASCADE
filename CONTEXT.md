...
- `type: "propagation"` — stores `event_ids` (every Event applied since the last Propagation — a user may stack several before running one, requirements §12.3a), `scenario_before`, optional `scenario_after`, optional `after_temporal_jump`, and optional `propagation_result`. The user saves one by clicking "Save to Scorecard" after a Propagation. See requirements §12.
...
**Network Importer**:
A pure transformation that converts an external network format into a CASCADE ProjectBundle — parse, reduce, map; never persist, never propagate. Each format lives in its own `CASCADE-backend/core/importers/<format>/` subpackage. The first is the EPANET `.inp` water-network importer (`core/importers/inp/`, `POST /api/import/inp`, ADR-0012): reservoirs/tanks become Source nodes (supply always the sum of their outgoing pipe capacities), demanding junctions become Service nodes, pumps and valves become inline nodes carrying the Requisite categories `pumping`/`valve` (a pump always imports as operational regardless of its `.inp` status — a working-condition baseline where "not currently scheduled" isn't "broken"; real pump failure is a Hazard, applied like any other), pipes become edges with capacity from each pipe's own peak simulated velocity (not a preset constant) and direction read off the SIGN of that same simulated flow (not guessed from graph topology) — a pipe carrying meaningful flow both ways across the sweep (e.g. a loop pipe reversing under stress) becomes two independently-capacitated edges sharing the pipe's one physical capacity between them, rather than being forced onto a single, possibly wrong, direction. Node/edge functionality values and the emitted `functionality_scale` are sized by a settable `n_levels` (default 3, matching the app default exactly). **Skeletonization** (WNTR) reduces a network to a node budget while conserving demand mass; the **hydraulic priority sweep** derives flow-allocation `priority` from pressure-driven WNTR solves so the engine's scarcity shedding order emulates real hydraulics; a second sweep (same mechanism) derives per-pipe capacity from peak simulated velocity under stress. Every Tank with a valid backup profile contributes to ONE shared "All Tanks — Running on Reserve" Disservice Event that starts every covered tank's backup countdown at once (sets `functionality_time = backup_duration` per tank) — a repeatable way to test the tanks' finite reserve without reconstructing a whole upstream-failure chain first. A network with any pumps gets one shared "Blackout" Hazard (full vulnerability on every pump, since a power outage takes them all out together — not a button per pump) and a "Demand Surge" Disservice that doubles demand for the top 10% of consumers by import-time demand. Import has two modes: **Replace project** (wholesale, like a local file load) and **Add as extra canvas** (merges the imported canvas/nodes/edges into the current project, remapping any colliding element id, and merges new categories/graph_types/events into the current config rather than replacing it — an event sharing an existing id has its `attribute_mutations` unioned in, so the three scenario Events above correctly grow to cover every merged network).
...
- **"admin" is ambiguous — resolve by context.** In v1 all roles (`viewer`/`analyst`/`manager`/`admin`) are **platform** roles: `admin` = the deployment operator, managing every user (this is what `create_admin.py` and the admin-escalation guard refer to). A customer's team admin (**org admin**) is a *future, org-scoped* role that does not exist yet. An **Organization** is a tenant (a customer); a user belongs to at most one (`org_id`) or is a standalone individual. Tenancy is app-DB-owned and its schema is deferred until server Sync. See ADR-0011.

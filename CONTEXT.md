...
**Graph Diff**:
A field-level, invertible description of what changed between two Scenarios — the form an **Any Graph Update** takes in `update_history`, replacing the pair of whole GraphSnapshots that cost 97.5% of a project file. Schema-agnostic by construction, so an attribute a Rule gains under ADR-0015 stays undoable without anyone changing the differ. Undo applies it backwards against the live graph, so no snapshot needs rebuilding. Mechanics in ADR-0017.
...
**Analysis Heatmap**:
The colour overlay encoding an Analysis Metric's scores on the canvas (**Analysis Mode** — colours mean scores, not Functionality). Applied as soon as a metric finishes and repainted whenever the scores change under it, so the colours can never describe a weighting other than the one selected (ADR-0018). Applying one swaps the canvas legend's Functionality scale for the metric's own key, derived at apply time by `lib/analysis-legend.ts`. Cleared by Reset.

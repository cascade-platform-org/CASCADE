...
quantity awareness. Simulators are costly, data-hungry, and require detailed calibration per domain; topological
...
Models need both readings, for different dependencies. Whether a district still
...
We present a category-typed architecture for propagating service
disruptions through directed infrastructure networks. Nodes carry a
discrete functionality level, while categories specify whether a
dependency is logical or quantity-aware. Logical propagation models
requirements and redundancy using worst-of and best-of aggregation.
For consumable resources, a plug-in flow module allocates finite supply
among demand nodes using priority-tiered max-min fairness. Both
mechanisms share the same functionality scale and monotonic update rule,
which guarantees finite convergence. We instantiate the flow module for
water distribution, automatically importing EPANET networks and
comparing the resulting functionality estimates with pressure-driven
WNTR simulations. The module is intended as a light-data approximation,
not as a replacement for detailed hydraulic analysis.
...
solutions (Section~\ref{sec:importer}). (3) A validation of that module
against pressure-driven WNTR simulation used as the hydraulic reference on eight networks (Section~\ref{sec:validation}).
% ============================================================================
...
{\scriptsize (a) after the blackout event}\\[1pt]\includegraphics[width=\linewidth]{fig_office_blackout}
\end{minipage}\hfill
\begin{minipage}[b]{0.44\textwidth}\centering
{\scriptsize (b) under a shared digital fault}\\[1pt]\includegraphics[width=\linewidth]{fig_office_digital}
\end{minipage}
...
\caption{The running example as direct screenshots, both starting from an all-operational initial condition.
 In~(a) the cracked Electrical Source zeroes the electric supply, so the Office~PC
...
$1,\ldots,N$ ($1$ = critical, $N$ = full operation); small discrete
...
Two algorithms do the proposal functionality updates. The \emph{logical} propagation runs
for every node over all of its incoming edges as defined in
Section~\ref{sec:arch-logical}. Nodes without positive demands and labelled with a category typed \textsc{Requisite} is resolved by only
the logical propagation. While nodes with positive demands or with a category typed \textsc{SourceToDemands} are considered in the flow module,
which solves one capacitated allocation over the considered subgraph (Section~\ref{sec:arch-flow}). A node holding a proposal from both then
...
of Section~\ref{sec:arch-pipeline}.
  ...
  \emph{redundancy} (one healthy input in the same category suffices). The logical propagation proposal
  is the result of the following three steps:
  \begin{enumerate}
  ...
  \end{enumerate}
  We notice that each of the above is a deterministic function of the parent nodes'
  and edges' functionalities, making the proposal locally computable.
  ...
  \subsection{Flow Propagation Module}\label{sec:arch-flow}

  A category typed \textsc{SourceToDemands} represents a service based on a resource that is sourced, carried and
  consumed in continuous amounts. The resource behaviour can differ depending on the domain: it may be governed by physical laws (like in a water distribution system),
  or by contractual agreements in a supply chain.
  This section evaluates how closely the hydraulically informed flow module reproduces pressure-driven WNTR 
  results. The importer derives the module's graph structure and capacities from EPANET 
  data and auxiliary hydraulic solutions; the resulting abstraction is then compared with PDD-WNTR 
  simulations of the same networks under the tested failures.


The algorithm's goal is to determine, for every consuming node, the largest share of 
its declared demand the network's scarce supply can deliver once higher-priority 
consumers are served in full, dividing any remaining shortage among equal-priority 
consumers as evenly as the network's capacities permit and turning the result into 
a per-node functionality estimate.
  For a typed \textsc{SourceToDemands} category, the module extracts the subgraph 
  (induced by all nodes with positive demands or labelled with that category)
  ...
  \emph{service} nodes only consume it, while edges carry a capacity. The module solves this on an auxiliary
  ...
  auxiliary graph by \emph{priority-tiered max-min fairness}~\cite{nace_tutorial_2006}: higher tiers
  served fully first, equal-priority consumers sharing any shortage as evenly as
  the network physically allows. For each consuming node, the delivered
  ...
  (Table~\ref{tab:familydetect}).} Reachability is blind to any degradation short of
  ...
  under \emph{source failure}--- reachability recovers just $0.54$
...
$n$-weighted mean of the four family rows. \emph{crit.} is the share of scored
junctions that are truly critical. FMS is demand-weighted agreement with WNTR on
\emph{how much} service each junction keeps ($1$ = exact). Reachability scores
precision $1.00$ by construction under the stated connectivity assumptions.}
\label{tab:familydetect}
\setlength{\tabcolsep}{4pt}
\small
\begin{tabular}{@{}lrr@{\hskip 1.1em}ccc@{\hskip 1.1em}cc@{\hskip 1.1em}cc@{}}
\toprule
& & & \multicolumn{3}{c}{Mean FMS}
...
\emph{Imprecision analysis.} In depth analysis reveals that the imprecisions are generally \emph{over-warning}: recall holds
...
Checking that regime needs a simulator that tracks the network moment by moment through the failure. \emph{Computational Cost.} The module runs slower than a direct WNTR solution (Supp.~Mat.~S8).

\emph{Future work.} The flow module is one plug-in for the proposed
architecture. Power, transport and telecommunications require their own 
domain-specific validation against established simulators which may suggest different approaches.
The logical layer provides the common coupling mechanism, but its
predictions require inspection and comparison with domain experts rather
than physical ground truth. Future work will therefore encode real
organizational and infrastructure systems and assess the resulting
propagation rules with multiple experts.

\emph{Supplementary material.} S1 describes the importer, S2 the
experimental protocol and metric conventions, S3 the required inputs,
S4 the per-network results, S5 the capacity and priority analyses, S6
the ground-truth sensitivity analysis, S7 unsuccessful alternatives,
and S8 the computational-cost measurements.


% ============================================================================
\section{Conclusion}\label{sec:conclusion}
We presented a category-typed propagation architecture in which logical
and quantity-aware dependencies share a discrete functionality scale
and a monotonic update rule. Its water flow module imports EPANET
networks, allocates capacitated supply by fair sharing, and reproduces
PDD-WNTR functionality with a mean Functionality Match Score of 0.92 across eight networks
and four failure families. Compared with reachability, it substantially
improves critical-junction recall for capacity-targeted and source
failures. These results support the use of the architecture as a
light-data approximation of disservice cascades.

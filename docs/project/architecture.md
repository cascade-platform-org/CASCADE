...
- **`lib/geo-utils.ts`** — the **GeoAnchor projection**: exact Web Mercator (`anchorFlowToGeo`, `computeMapTarget`). One seam converts flow ↔ geo, so `node.geo`-on-drag and the map camera can never use disagreeing projections. See CONTEXT.md → *GeoAnchor*.
...
5. `api-client` sends `POST /api/propagate` with the payload. The session's
   httpOnly cookie carries the JWT — the client never holds or attaches one.

...
Body `{ filename, content, target_nodes?, source_crs?, demand_mode?, derive_priorities?, n_levels?, capacity_margin?, max_velocity? }` — `content` is the raw `.inp` text; `target_nodes` defaults to the caller's Entitlement `max_nodes` (300 for unbounded roles); `source_crs` defaults to `EPSG:3004`; `n_levels` defaults to 3 (functionality scale size — pass the current project's own `functionality_scale.length` when merging the result into an existing project, so imported values land on that scale); `capacity_margin` defaults to 2.0 (uniform multiplier on sweep-derived pipe capacity — lower is more conservative, higher more optimistic); `max_velocity` defaults to null/uncapped (optional physical ceiling in m/s on the margined pipe velocity, ~2.5–3 for water mains, so no margin implies an unphysically fast pipe). Source `supply_capacity` is always the sum of a Reservoir/Tank's outgoing pipe capacities — not a request parameter.

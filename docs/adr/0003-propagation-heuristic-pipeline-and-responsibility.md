...
2. **`backup` deferral — per category.** Engages on **any** proposed drop (not only a drop to critical) when a binding category has backup, and covers **only its own category**: the backed categories' contribution is deferred into `functionality_time`, while the proposal is **re-composed from the unbacked categories alone and that level still commits this round** (a reserve for `digital` does not keep the lights on when `power` fails — previously the whole merged drop was held, wrongly masking concurrent unbacked failures). Only when *every* degrading category is backed does the node hold its current Functionality. The countdown is the **worse (shortest) reserve**: `functionality_time = min(backup_duration over backed binding categories)`, and an **already-running countdown is tightened** (`min(existing, pending)`), never extended or refreshed. The deferred drop is realised by a future Temporal Jump (see CONTEXT.md → *Functionality Time*). Backup is consulted on the binding (worst) categories; backup on a non-binding category does not protect against a different category's failure. Regression tests: `test/test_engine_guards.py` (per-category deferral block).
